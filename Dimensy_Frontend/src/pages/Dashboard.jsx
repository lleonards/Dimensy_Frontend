...
export default function Dashboard() {
  ...
  const startSubscriptionCheckout = async () => {
    try {
      setShowPlans(false);
      const res = await api.post('/api/stripe/create-checkout', { planKey: 'ilimitado' });
      if (res.data?.url) {
      ...
      if (err.response?.data?.requiresStripeConfig) {
        setShowPlans(true);
      }
      toast.error(err.response?.data?.error || 'Não foi possível abrir o checkout da assinatura.');
      throw err;
    }
  };
  ...
  const openGenerateModal = async () => {
    ...
    if (!ilimitado && credits <= 0) {
      try {
        ...
        // fallback visual tratado pela própria função
      }
      return;
    }
    setProjectName('');
  ...
  const handleConfirmGenerate = async () => {
    ...
    try {
      ...
      const report = {
        projeto_id: res.data.projeto?.id,
        creditos_consumidos: res.data.projeto?.creditos_consumidos ?? res.data.relatorio?.creditos_consumidos ?? 0,
        storage_json_url: res.data.projeto?.arquivo_json_url || res.data.relatorio?.storage_json_url || null,
        storage_json_path: res.data.projeto?.arquivo_json_path || res.data.relatorio?.storage_json_path || null,
        ...res.data.relatorio
      };
      ...
      if (err.response?.data?.semCreditos) {
        try {
          ...
          // fallback já tratado visualmente pela função acima
        }
      } else {
  ...
  const handleOpenHistoryProject = async (projectId) => {
    ...
    try {
      ...
      const report = {
        projeto_id: res.data.projeto.id,
        creditos_consumidos: res.data.projeto.creditos_consumidos ?? 0,
        storage_json_url: res.data.projeto.arquivo_json_url || null,
        storage_json_path: res.data.projeto.arquivo_json_path || null,
        ...res.data.projeto.resultados
      };
      setCurrentProject(report);
      saveLastGeneratedProject(report);
  ...
  const handleOpenStorageFile = (url) => {
    if (!url) {
      toast.error('O arquivo salvo no Supabase Storage ainda não está disponível para este projeto.');
      return;
    }
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#0d0d0f' }}>
      {showPlans && <PlansModal onClose={() => setShowPlans(false)} onStartCheckout={startSubscriptionCheckout} />}
      <ProjectNameModal
      ...
      <div style={{ background: 'linear-gradient(135deg, #0d0d0f 0%, #111120 50%, #141428 100%)', borderBottom: '1px solid #1e1e2e', padding: '40px 24px 32px', textAlign: 'center' }}>
        ...
        <div style={{ display: 'inline-flex', alignItems: 'center', gap: '10px', flexWrap: 'wrap', background: ilimitado ? 'rgba(76,175,130,0.08)' : credits === 0 ? 'rgba(224,82,82,0.08)' : 'rgba(74,127,165,0.08)', border: `1px solid ${ilimitado ? 'rgba(76,175,130,0.3)' : credits === 0 ? 'rgba(224,82,82,0.3)' : 'rgba(74,127,165,0.3)'}`, borderRadius: '30px', padding: '10px 20px', marginTop: '20px' }}>
          ...
          <span style={{ color: ilimitado ? '#4caf82' : credits === 0 ? '#e05252' : '#6ba3cc', fontWeight: '600', fontSize: '0.9rem' }}>
            {ilimitado ? 'Assinatura ativa com acesso ilimitado' : credits === 1 ? '1 crédito disponível para gerar projeto' : `${credits} créditos disponíveis para gerar projetos`}
          </span>
        </div>
      </div>
      ...
      <div style={{ maxWidth: '1180px', margin: '0 auto', padding: '32px 24px 48px' }}>
        <div className="card" style={{ marginBottom: '28px' }}>
          ...
          <div className="result-grid" style={{ marginBottom: '18px' }}>
            ...
            <div className="result-item"><div className="value">{ilimitado ? 'Assinante' : '1 crédito'}</div><div className="label">Consumo por geração</div></div>
          </div>
                  ...
                  <div key={project.id} style={{ background: '#141418', border: '1px solid #2a2a38', borderRadius: '12px', padding: '16px' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', gap: '12px', flexWrap: 'wrap', alignItems: 'flex-start' }}>
                      ...
                      <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
                        {project.arquivo_json_url && (
                          <button onClick={() => handleOpenStorageFile(project.arquivo_json_url)} className="btn-secondary" style={{ padding: '8px 12px', fontSize: '0.82rem' }}>
                ...
                {creditHistory.slice(0, 8).map((item) => (
                  <div key={item.id} style={{ background: '#141418', border: '1px solid #2a2a38', borderRadius: '10px', padding: '12px 14px' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', gap: '12px', marginBottom: '6px', flexWrap: 'wrap' }}>
                      <strong style={{ color: '#fff', fontSize: '0.9rem' }}>{item.ferramenta || 'Uso de crédito'}</strong>
                      <span className={`badge ${item.creditos_depois > 0 || ilimitado ? 'badge-green' : 'badge-red'}`}>
                        {ilimitado ? 'Ilimitado' : `${item.creditos_antes} → ${item.creditos_depois}`}
                      </span>
                    </div>
        ...
        {currentProject && (
          <div className="card">
            <SectionHeader
              ...
              action={(
                <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
                  {currentProject.storage_json_url && (
                    <button onClick={() => handleOpenStorageFile(currentProject.storage_json_url)} className="btn-secondary" style={{ padding: '8px 12px', fontSize: '0.82rem' }}>
                      <ExternalLink size={14} /> JSON
                    </button>
                  )}
            ...
            <div className="result-grid" style={{ marginBottom: '20px' }}>
              ...
              <div className="result-item"><div className="value">{currentProject.storage_json_url ? 'Storage' : currentProject.projeto_id ? 'Salvo' : 'Local'}</div><div className="label">Status</div></div>
            </div>
