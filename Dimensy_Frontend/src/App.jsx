...
// FIX: Layout now loads company slug dynamically to build the correct public page link
      ...
      <header className="border-b border-slate-200 bg-white">
        <div className="mx-auto flex max-w-7xl items-center justify-between px-4 py-4">
          ...
          <div className="flex items-center gap-4">
            {companySlug && (
              // FIX: Public page route changed to /p/:slug to avoid conflict with dashboard routes
...
// FIX: SettingsPage now handles the "no company yet" case (new user onboarding)
...
function ProfilePage({ token }) {
  ...
  useEffect(() => {
    api('/dashboard/profile', {}, token).then((res) => setProfile(res.profile)).catch((err) => {
      setIsError(true);
      setMessage(err.message);
    });
  }, [token]);
...
// FIX: Public page now uses /p/:slug route (avoids conflict with dashboard routes)
// FIX: Branch deselection now clears associated services
// FIX: Added error state when company is not found
function PublicPage() {
  ...
  // FIX: When toggling a branch off, removes services that belong to that branch
  const toggleBranch = (branchId) => {
    setForm((current) => {
      ...
      if (removing && data) {
        const branchServiceIds = data.company.services
          .filter((s) => s.branch_id === branchId)
          .map((s) => s.id);
        return {
          ...current,
          branchIds: newBranchIds,
          serviceIds: current.serviceIds.filter((id) => !branchServiceIds.includes(id)),
        };
      }
...
export default function App() {
  return (
    <Routes>
      ...
      {/* FIX: Public page uses /p/:slug to avoid conflict with dashboard routes like /leads, /ramos, etc. */}
