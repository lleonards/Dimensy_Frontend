    ...
    registros.forEach((registro, index) => {
      ...
      if (toolKey === 'tomadas') {
        sections.push({
          ...
          lines: [
            `Item da norma usado: ${resultado.norma?.item || '-'}`,
...
export async function exportReportAsDocx(report) {
  ...
  const children = [heading('Relatório Dimensy', HeadingLevel.TITLE)];
  ...
  const safeName = (report?.nome_projeto || 'relatorio-dimensy').replace(/[^a-z0-9-_]+/gi, '-').toLowerCase();
...
export async function exportReportAsPdf(report) {
  ...
  doc.setFontSize(20);
  doc.text('Relatório Dimensy', 40, y);
  y += 28;
